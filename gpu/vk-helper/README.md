...
звука при отказе, кириллические пути, quit, конец stdin, смерть родителя (код 5). Устройств
на раннере нет, поэтому подробности устройства проверяются тем же скриптом на машине с
видеокартой: схема полей, сверка со строкой ggml и `GGML_VK_DISABLE_*`. Таблица
