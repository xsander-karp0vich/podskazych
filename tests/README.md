...
`context/store.ts` (папку хранилищу передают снаружи, поэтому тест гоняет его на временной папке), `zip.ts`
и `gpu/download.ts` — загрузку пакета для видеокарты тест гоняет на своём HTTP-сервере на 127.0.0.1 и временной папке;
`llm/claude.ts` — прокси из окружения (`tests/proxy.test.ts`: глобальный диспетчер undici живёт до конца процесса, поэтому отдельный файл).
