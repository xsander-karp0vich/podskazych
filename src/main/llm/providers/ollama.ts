 ...
 *
 * Что здесь важно:
 * - Запросы идут через node:http со своим агентом, а не через глобальный fetch. Глобальный прокси
 *   undici (configureProxy в claude.ts) локальные адреса теперь пропускает мимо, но свой агент
 *   не зависит от прокси вовсе: Ollama всегда на этом компьютере, и лишнее звено ей не нужно.
