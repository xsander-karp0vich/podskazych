...
Работает у всех. Распознавание речи идёт на видеокарте NVIDIA, на AMD и Intel — через Vulkan после скачивания пакета ускорения, а без них — той же моделью на процессоре. Подсказки пишет модель, которая у вас уже есть: Claude, ChatGPT, Cursor или Gemini — через их официальные программы на вашей подписке, — либо локальная модель через Ollama без интернета.
...
- **Свой промпт и команды.** Встроенную инструкцию можно заменить своей — например, ролью кандидата на техническом собеседовании. Команды из промпта вида `/разбор` становятся кнопками в строке запроса: нажали — команда ушла модели вместе с расшифровкой. Там же прикрепляются файлы для контекста — резюме, описание проекта, вакансия в PDF, DOCX, TXT или MD: их текст модель учтёт в подсказках от вашего лица.
...
**Видеокарта AMD или Intel.** В «Настройки → Основные» появится строка «Ускорение на видеокарте» с кнопкой «Скачать · 890 МБ». Пакет — помощник на whisper.cpp с Vulkan и модель `ggml-large-v3-turbo-q8_0` — скачивается один раз из релиза [`gpu-vulkan-v1`](https://github.com/xsander-karp0vich/podskazych/releases/tag/gpu-vulkan-v1) в `%LOCALAPPDATA%\Podskazych\gpu`, с проверкой контрольных сумм и докачкой после обрыва. При старте сессии видеокарта проходит короткую проверку скорости: не успевает — распознавание остаётся на процессоре, и строка скажет почему. Установщик от этого не растёт, удалить пакет можно там же.
...
| Что | Где |
| --- | --- |
| **Остаётся на компьютере** | звук обоих каналов и распознавание речи — whisper работает локально; журнал созвонов и извлечённый текст файлов контекста — в папке данных приложения |
| **Уходит в выбранный сервис** | текст запроса к модели: расшифровка, ваш вопрос, найденное в базе и текст прикреплённых файлов контекста — сами файлы остаются на компьютере; снимки экрана — только по кнопке «Скриншот» или «+»; запрос второму агенту, если он включён. С Ollama в сеть не уходит ничего |
| **Скачивается по кнопке** | пакет ускорения для видеокарт AMD и Intel — с GitHub, только когда вы нажмёте «Скачать». Это единственный сетевой запрос самого приложения: ничего о вас и о созвонах в нём не уходит |
...
Установщик собирается командой `npm run dist`: она проверяет встроенную базу вопросов и что контрольные суммы пакета ускорения вписаны в `src/shared/gpuPack.ts`, собирает переносимое окружение Python с моделью в `build/stt` (`scripts/build-stt-runtime.ps1`) и упаковывает всё в `dist/Podskazych-Setup-x64.exe`.
...
| `COPILOT_STT_DEVICE` | `cpu` — принудительно распознавать на процессоре, `cuda` — только на видеокарте NVIDIA, `vulkan` — на видеокарте через пакет ускорения, даже если проверка скорости выбрала бы процессор |
| `COPILOT_GPU_PACK_DIR`, `COPILOT_GPU_PACK_URL` | свой каталог пакета ускорения и свой адрес его файлов вместо релиза `gpu-vulkan-v1` |
